...
/*
#define icmdReadVersion         0x01 // Read Version
#define icmdReadData            0x02 // Sensor Read Data
#define icmdFlashStart          0x03 // Start recording data into flash
#define icmdFlashReadPos        0x04 // Read Flash at position
#define icmdFlashEraseFull      0x05 // Erase Flash
#define icmdFlashSetSampleSize  0x06 // Set flash sample size.
#define icmdFlashStatus         0x07 // Get Status of flash reading. Read percent 100 means done.
#define icmdFlashGetData        0x08 // Send the flash memory buffer.
*/
enum {
    icmdReadVersion=0x01,
    icmdReadData,
    icmdFlashStart,
    icmdFlashReadPos ,
    icmdFlashEraseFull ,
    icmdFlashSetSampleSize,
    icmdFlashStatus,
    icmdFlashGetData
};
        ...
        if(bit_check(CAN_RECV.FLAGS,CAN_F_NEW))
        {
            ...
            switch(CAN_CMD_REQUEST)
            {
                 ...
                 case icmdFlashStatus:
                     ...
                     CANSendMSG(CANID_tmp,CAN_RESP);
                     break;
                 case icmdFlashGetData:
                     CANVAL_tmp = FlashSampleSize; //Send size of sample with first response.
                     CAN_RESP[4] = (uint8_t)(CANVAL_tmp >> 24);
                     CAN_RESP[5] = (uint8_t)(CANVAL_tmp >> 16);
                     CAN_RESP[6] = (uint8_t)(CANVAL_tmp >> 8);
                     CAN_RESP[7] = (uint8_t)(CANVAL_tmp);
                    ...
                    CANVAL_tmp = 0x0; //send zero to end stream.
                    CAN_RESP[4] = (uint8_t)(CANVAL_tmp >> 24);
                    CAN_RESP[5] = (uint8_t)(CANVAL_tmp >> 16);
                    CAN_RESP[6] = (uint8_t)(CANVAL_tmp >> 8);
                    CAN_RESP[7] = (uint8_t)(CANVAL_tmp);
